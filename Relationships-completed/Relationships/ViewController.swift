...
//        sleep(5)
    ...
    private func findBooks(by author: Author?) {
        ...
        // BELOW: Query Implementation
        let authorName: String = author.name ?? "no name"
        PFQuery(className: Book.parseClassName())
            .whereKey("author", equalTo: author)
            .findObjectsInBackground { (results, error) in
                print("\nQUERY #1: ")
                if let books = results as? [Book] {
                    print(authorName + " has written the following books: ")
                    for b in books {
                        print(b.title ?? "")
                    }
                }
    ...
    private func booksRead(by reader: Reader?) {
        ...
        // BELOW: Query Implementation
        let readerName: String  = reader.name ?? "no name"
        reader.hasRead.query().findObjectsInBackground { (results, error) in
            print("\nQUERY #2: ")
            if let books = results as? [Book] {
                print(readerName + " has read the following books:")
                for b in books {
                    print(b.title ?? "")
                }
            }
    ...
    private func readersHaveRead(_ book: Book?) {
        ...
        // BELOW: Query Implementation
        let bookTitle: String = book.title ?? "No title"
        PFQuery(className: Reader.parseClassName())
            .whereKey("hasRead", equalTo: book)
            .findObjectsInBackground { (results, error) in
                print("\nQUERY #3: ")
                if let readers = results as? [Reader] {
                    print("The following readers have read " + bookTitle + ": ")
                    for r in readers {
                        print(r.name ?? "")
                    }
                }
